...
def _build_image_prompt_for_zone(zone):
    # A fallback image prompt generator that ensures every suggestion has an image prompt
    prompts = [
        "photorealistic",
...
def run_gemini_suggestions():
    ...
    try:
        ...
        # The Gemini response object may expose .text or .candidates. Try multiple strategies to parse.
        suggestions = []
        parsed = None
        response_text = None
        try:
            ...
            try:
                # some SDK responses keep a .candidates list
        ...
        # If parsing failed or suggestions empty, build fallback suggestions per hotspot
        if not suggestions:
            ...
            for _, z in hotspots.iterrows():
                detailed = (f"Immediate measures: deploy mobile misting fans and temporary shaded canopies.\n"
                            f"Mid-term: install cool roofs and increase tree canopy along main walkways.\n"
                            f"Community: run heat awareness and distribution of hydration kits.")
                img_prompt = _build_image_prompt_for_zone(z)
                suggestions.append({"zoneId": int(z['id']), "detailed_suggestion": detailed, "image_prompt": img_prompt})

        # Apply suggestions to zones
        zones_df = st.session_state.zones.copy()
        for s in suggestions:
            try:
                zid = int(s.get('zoneId') or s.get('zone_id') or s.get('id'))
            except Exception:
                # If zoneId is missing, try matching by name (best effort)
                zid = None
            suggestion_text = s.get('detailed_suggestion', '').replace('\n', '<br>') if s.get('detailed_suggestion') else ''
            image_prompt = s.get('image_prompt') if s.get('image_prompt') else ''
            # if image_prompt empty, create one based on zone info
            if not image_prompt:
            ...
            # build an Unsplash query-safe URL
            ...
            if zid is not None and zid in zones_df['id'].values:
                zones_df.loc[zones_df['id'] == zid, ['suggestion', 'image_url']] = [suggestion_text, image_url]
            else:
                # try match by name included in suggestion (very best-effort)
                for _, z in zones_df.iterrows():
                    if z['name'].lower() in suggestion_text.lower():
                        zones_df.loc[zones_df['id'] == z['id'], ['suggestion', 'image_url']] = [suggestion_text, image_url]
        st.session_state.zones = zones_df
            ...
            with cols[i % 4]:
                with st.container():
                    ...
                    if zone['mitigation_intensity'] > 0:
                        st.progress(zone['mitigation_intensity'], "Intensity: {zone['mitigation_intensity']:.2f}")
                ...
                with st.container():
                    ...
                    if row['image_url']:
                        try:
                            ...
                            # fallback: show a tiny placeholder
