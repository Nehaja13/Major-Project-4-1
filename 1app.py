...
try:
    ...
    st.info("Ensure you have a file at `.streamlit/secrets.toml` in your project's root folder and that you are running `streamlit run` from that root folder.")
    st.code("""
...
try:
    ...
    st.error(f"Failed to initialize OpenRouteService client. Check your ORS_API_KEY. Error: {e}")
    st.stop()
...
ZONE_GRAPH = { 1: [10, 11, 7], 2: [5, 12], 3: [12], 4: [6], 5: [2, 12, 10], 6: [4, 9, 8, 11], 7: [1, 10], 8: [6], 9: [6, 11], 10: [1, 5, 7, 11], 11: [1, 9, 10, 6], 12: [2, 3, 5] }
...
def run_gemini_suggestions():
    ...
    details = "\n".join([f'- Zone ID: {z["id"]}, Name: "{z["name"]}", Temp: {z["temperature"]:.1f}°C, AQI: {z["aqi"]}' for _, z in hotspots.iterrows()])
    prompt = f'You are an expert urban planning AI for Hyderabad. For each zone below, provide a tailored mitigation strategy. Respond in a single JSON object: {{"suggestions": [{{"zoneId": <int>, "detailed_suggestion": "<str>", "image_prompt": "<str>"}}]}}. Use \\n for newlines.\n{details}'
    try:
...
def fetch_aqi_from_api(lat, lon):
    ...
    try:
        ...
        if data.get("status") == "ok":
            return (data["data"].get("aqi"), data["data"]["iaqi"].get("pm25", {}).get("v"), data["data"]["iaqi"].get("pm10", {}).get("v"))
...
def mock_lstm_forecast(past_aqi):
    ...
    return [max(10, last_value*0.95 + 75*0.05 + 15*np.sin(2*np.pi*((len(past_aqi)+i)%24)/24) + np.random.normal(0,3)) for i in range(168)]
...
if 'zones' not in st.session_state:
    ...
    with st.spinner("Fetching live baseline data..."):
        ...
        if not pd.notna(aqi):
            ...
            st.session_state.baseline_aqi, st.session_state.baseline_pm25, st.session_state.baseline_pm10 = 110, 45, 80
        else:
            st.session_state.logs.append("Live baseline data fetched.")
            st.session_state.baseline_aqi, st.session_state.baseline_pm25, st.session_state.baseline_pm10 = aqi, pm25, pm10
    
    # FIX: Ensure all session state variables are initialized
...
with tab1:
    ...
    with col2:
        ...
        if st.button("Reset Scenario", type="secondary", use_container_width=True):
            for key in ['stage', 'error', 'route_result', 'cluster_result', 'is_navigating', 'user_location']:
...
with tab2:
    ...
    # FIX: Use pd.notna() for robust checking of None or NaN
    if pd.notna(latest_aqi):
        ...
        c2.metric(label="PM10", value=f"{latest_pm10} µg/m³" if pd.notna(latest_pm10) else "N/A")
    else:
    ...
    fig = px.bar(forecast_df, x='Day', y='Average AQI', title="7-Day Average AQI Forecast", text_auto='.2s')
...
with tab3:
    ...
    if st.button("🗺️ Find Routes", type="primary", use_container_width=True):
        if start_name == end_name:
            ...
            with st.spinner("Calculating optimal waypoints and fetching road paths..."):
                ...
                st.session_state.route_result = {
                    "safe_path_coords": safe_path_coords, "safe_aqi": safe_aqi, "safe_temp": safe_temp,
    ...
    if st.session_state.get('route_result'):
        ...
        if st.session_state.is_navigating and st.session_state.user_location:
            folium.Marker(
                (st.session_state.user_location['latitude'], st.session_state.user_location['longitude']),
                popup="Your Location",
        ...
        st.markdown("##### 🏆 Safest Route Details")
        ...
        if st.session_state.is_navigating:
            with nav_col2:
                st.info("Live navigation active... Polling location.")
            time.sleep(5)
            location = streamlit_geolocation()
            if location and location.get('latitude'):
...
with tab4:
    ...
    if st.session_state.get('cluster_result') is not None:
        ...
        fig = px.scatter(clustered_df, x='aqi', y='temperature', color='cluster', color_continuous_scale=px.colors.qualitative.Vivid,
                         hover_name='name', title="Zone Clusters by Temperature and AQI")
        st.plotly_chart(fig, use_container_width=True)
        ...
        for _, zone in clustered_df.iterrows():
            ...
            folium.Marker([zone['lat'], zone['lon']], popup=f"<strong>{zone['name']}</strong><br>Cluster: {'Outlier' if cluster_id == -1 else cluster_id}",
                          icon=folium.Icon(color=color)).add_to(cluster_map)
